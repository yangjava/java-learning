...
public class DbutilsHandlerTest {
...
/*          Object[] result = runner.query(sql, new ArrayHandler());
	...
	//自定义ColumnListHandler处理器需要实现接口 ResultSetHandler
	@Test
    public void testMyColumnListHandler() throws SQLException {
        QueryRunner runner = new QueryRunner(DataSource.getDataSource());
        String sql = "select * from user ";
        List<String> list = (List<String>)runner.query(sql, new MyColumnListHandler("username"));
        for (int i = 0; i < list.size(); i++) {
        	 System.out.println(list.get(i));
		}
    }
